  ...
  try {
    ...
    if (!thaiId || thaiId.length !== 13) {
      return NextResponse.json(
        { source: 'election-pm', error: 'Invalid Thai ID' },
        { status: 400 }
      );
    }

    const API_URL = `https://boraservices.bora.dopa.go.th/api/election/v1/nenuit3/${thaiId}`;
    ...
    console.error('Election PM API Error:', error);
    return NextResponse.json(
      {
        source: 'election-pm',
        error: error.message || 'Failed to fetch from election PM API',
      },
      { status: 500 }
    );
  }
